 ...
 * @brief DOM node implementation: tree mutation, navigation, query API.
...
DOMNode* DOMNode::cloneNode(bool deep) const {
    ...
    if (nodeType == NodeType::Document) {
        ...
        copy->ownerDocument = static_cast<Document*>(copy);
    } else if (!deep) {
        copy->ownerDocument = nullptr;
    } else {
        copy->ownerDocument = ownerDocument;
    }

    if (deep) {
        Document* docOwner = (nodeType == NodeType::Document)
            ? static_cast<Document*>(copy)
            : nullptr;
        for (const DOMNode* c = firstChild; c; c = c->nextSibling) {
            DOMNode* cc = c->cloneNode(true);
            cc->parent = copy;
            cc->ownerDocument = docOwner ? docOwner : copy->ownerDocument;
